...
############################################################################
  ...
  def __init__(self, instName='DIRAC', projectName='DIRAC', globalDefaultsURL=None):
    ...
    if globalDefaultsURL:
      ...
      self.globalDefaultsURL = "http://dirac2.cc.kek.jp/dirac/globalDefaults.cfg"
...
def createBashrc():
  ...
  try:
    ...
    if not os.path.exists(bashrcFile):
      ...
      # Add the lines required for Rucio
      lines.extend(['# Rucio client',
                    '_PYTHONWARNINGS=ignore',
                    '[ -n "$PYTHONWARNINGS" ] && export PYTHONWARNINGS=${PYTHONWARNINGS}","$_PYTHONWARNINGS || export PYTHONWARNINGS=$_PYTHONWARNINGS',
                    'export RUCIO_HOME=/cvmfs/belle.cern.ch/grid/rucio-clients/pro',
                    'export RUCIO_AUTH_TYPE=x509_proxy',
                    'export PYTHONPATH=$PYTHONPATH:$RUCIO_HOME/lib/python2.7/site-packages'])
      # RUCIO_ACCOUNT will be set by RucioFileCatalog
...
def createCshrc():
  ...
  try:
    ...
    if not os.path.exists(cshrcFile):
      ...
      # Add the lines required for Rucio
      lines.extend(['# Rucio client',
                    'set _PYTHONWARNINGS=ignore',
                    '( test $?PYTHONWARNINGS -eq 1 ) && setenv PYTHONWARNINGS ${PYTHONWARNINGS}","$_PYTHONWARNINGS || setenv PYTHONWARNINGS $_PYTHONWARNINGS',
                    'setenv RUCIO_HOME /cvmfs/belle.cern.ch/grid/rucio-clients/pro',
                    'setenv RUCIO_AUTH_TYPE x509_proxy',
                    'setenv PYTHONPATH ${PYTHONPATH}:${RUCIO_HOME}/lib/python2.7/site-packages'])
      # RUCIO_ACCOUNT will be set by RucioFileCatalog
